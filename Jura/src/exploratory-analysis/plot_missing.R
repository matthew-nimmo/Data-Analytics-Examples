...
#'
#' @references \url{https://jev-pankov.com/2017/11/15/visualize-missing-values-in-r/}
#' source found here: \url{https://github.com/jevgenij-p/blog/blob/master/Missing%20Values%20Plot/MissingValues.R}
#'
#' @return a ggplot object
#'
plot_missing <- function(data, by=NULL, seg=10, col=NULL, horiz=TRUE) {
  library(ggplot2)
  library(reshape)
  ...
  sort_index <- sort.int(-missing_intensity[, 1], index.return=TRUE)$ix
  ...
  rnames <- factor(rownames(missing_intensity),
                   levels=rev(rownames(missing_intensity)), ordered=FALSE)
  ...
  if (horiz) {
    ...
    ggplot(missvalues, aes(x=Columns, y=variable, fill=value)) +
      geom_tile() +
      ylab('Segments') +
      ggtitle(paste0('Missing Value Rate (', general_rate, '%)')) +
      scale_fill_gradientn(name='Missing\nvalue', colours=new.palette) +
